...
install_dependencies() {
    ...
    local dependencies=("vllm" "python-dotenv" "toml" "openai" "triton==2.1.0" "wheel" "packaging" "psutil" "flash-attn")
...
validateVram() {
    ...
    if [[ "$heurist_model_id" == *"mixtral-8x7b-gptq"* ]] && [ "$available_mb" -gt 32000 ]; then
        ...
        local gpu_memory_util=$(echo "scale=2; (22000-1000)/$available_mb" | bc)
    else
        local gpu_memory_util=$(echo "scale=2; (22000-1000)/$available_mb" | bc) # Default value or handle other cases as needed
    fi
