...
def generate(base_config, server_config, miner_id, job_id, prompt, temperature, max_tokens, seed, stop, use_stream_flag, model_id, request_latency, session):
    ...
    try:
        if use_stream_flag:
            ...
            res = session.post(url, json=result)
...
def worker(miner_id_list):
    ...
    while True:
        ...
        try:
            ...
            job, request_latency = send_miner_request(base_config, miner_id, base_config.served_model_name, session)
            if job is not None:
...
def periodic_send_model_info_signal(base_config, miner_ids, last_signal_time):
    while True:
